...
import {AddResponseRule} from "./AddResponseRule.js";
...
it("renders correctly without response rules", ()=> {
    const form = renderer
    .create(
        <TestWrapper store={store}>
            <AddResponseRule {...props1} store={store} />
        </TestWrapper>
    )
    .toJSON();
...
it("renders correctly with test response rule", ()=> {
    const form = renderer
    .create(
        <TestWrapper store={store}>
            <AddResponseRule {...props2} store={store} />
        </TestWrapper>
    )
    .toJSON();
    expect(form).toMatchSnapshot();
